...
<style>
    ...
    .highlight { background-color: #e8f5e9; padding: 10px; border-radius: 10px; margin-bottom: 10px; }
</style>
...
with col1:
    ...
    st.markdown("##### An Intelligent Screening System by **Team Dual Core**")
with col2:
...
with st.sidebar.expander("🆘 Emergency Helpline (BD)"):
    st.markdown("""
    **Kaan Pete Roi:** 📞 01779554391 / 01779554392  
...
def render_questions(tab, start_idx, end_idx):
    with tab:
        st.markdown(f"<div class='highlight'>Please answer questions {start_idx+1} to {end_idx}</div>", unsafe_allow_html=True)
        for i in range(start_idx, end_idx):
            q_text = q_labels[i]
            key_name = f"q_{i}_{st.session_state.reset}"
            val = st.select_slider(label=q_text, options=slider_options, value=slider_options[0], key=key_name)
            answers_map[i] = options_map[val]

render_questions(tab1, 0, 10)
render_questions(tab2, 10, 17)
render_questions(tab3, 17, 26)
            ...
            for i, cond in enumerate(conditions):
                ...
                with result_cols[i]:
                    ...
                    if is_healthy:
                        ...
                        st.progress(0)
                        st.caption(f"Risk Level: Low ({confidence:.1f}%)")
                        risk_scores.append((cond, 0))
                        healthy_count += 1
                    else:
                        st.error(f"**{display_label}**")
                        st.progress(int(confidence))
                        st.caption(f"Risk Level: High ({confidence:.1f}%)")
